          ...
          <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0">
            {categories.map((c) => (
              <button
                ...
                className={`whitespace-nowrap px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeCategory === c
        ...
        {filteredGames.map((game) => {
          ...
          return (
            <Link
              ...
              onClick={handleClick}
              className={`group relative h-96 rounded-2xl overflow-hidden border border-white/5 bg-[#1A1A1A] block transition-all duration-500 ${game.isActive
            ...
            >
              ...
              <div className="absolute inset-0">
                <img
                  src={game.bgImage}
                  alt={game.name}
                  className={`w-full h-full object-cover transition-transform duration-700 ${game.isActive ? 'group-hover:scale-105' : 'grayscale'
                    }`}
                />
              ...
              {/* Character Image */}
              {game.charImage && (
                <div className={`absolute inset-0 z-10 pointer-events-none transition-transform duration-700 ease-out origin-bottom-right ${game.isActive ? 'group-hover:scale-110' : 'grayscale'
                  }`}>
                  <img
                    src={game.charImage}
              ...
              <div className="absolute bottom-0 left-0 w-full p-6 z-20">
                <h3 className="text-2xl font-black text-white mb-1 tracking-tighter uppercase drop-shadow-lg">
                  {game.name}
                </h3>
